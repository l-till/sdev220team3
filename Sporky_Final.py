"""
Made by, in order of last name: Paul Deater, Bryce Harrison, and Logan till
Sporky App
For SDEV 220
"""
...
class OrderManager(ttk.Frame):
    ...
    def place_order(self):
        self.order_status.set(f"Order placed successfully for {self.current_table}")
        messagebox.showinfo("Order Status", f"Order placed successfully for {self.current_table}")
...
{str(sub_key['price'])}"""
    ...
    def UpdateItem(self):
        ...
        self.modify_menu_window.geometry("800x670")

        self.modify_menu_current_section ="Apps"

        modify_section_frame = tk.Frame(self.modify_menu_window, bg="gray85")
        modify_section_frame.grid(row=0, column=0, padx=5, pady=5)

        self.modify_detail_frame = tk.Frame(self.modify_menu_window)
        self.modify_detail_frame.grid(row=0, column=1, padx=5, pady=5)

        # Creates the buttons for the previously mentioned frame
        for section in self.menu.keys():
            section_button = tk.Button(modify_section_frame, text = section, command = lambda open_section = section: self.populate_modify_section(open_section), bg="seashell3",height=1,width=10)
            self.modify_menu_current_section = str(self.menu[section])
            section_button.pack(side=BOTTOM, expand=True)

        self.sub_modify_section_frame = tk.Frame(self.modify_menu_window, bg="gray85")
        self.sub_modify_section_frame.grid(row=1, column=1, padx=5, pady=5)

        self.sub_modify_section_frame_left = tk.Frame(self.sub_modify_section_frame, bg="gray85")
        self.sub_modify_section_frame_left.grid(row=0, column=0, padx=5, pady=5)

        self.sub_modify_section_frame_right = tk.Frame(self.sub_modify_section_frame, bg="gray85")
        self.sub_modify_section_frame_right.grid(row=0, column=1, padx=5, pady=5)

        self.left_frame_label = Label(self.sub_modify_section_frame_left, text = "Select an item then press Delete to remove.")
        self.left_frame_label.pack(side=TOP)

        self.current_select_item_label = Label(self.sub_modify_section_frame_left, text = "- No item selected yet -")
        self.current_select_item_label.pack(side=LEFT)

        self.create_button = tk.Button(self.sub_modify_section_frame_right, text = "Create",command = lambda :self.create_item(), bg="seashell3")
        self.create_button.pack(side=BOTTOM)

        self.left_frame_label = Label(self.sub_modify_section_frame_right, 
                                      text = f"""Select a section on the left, then select an item in that section 
then enter the name and price, next press Create.""", padx=5, pady=5)
        self.left_frame_label.pack(side=TOP)

        self.name_label = Label(self.sub_modify_section_frame_right, text = "Name:")
        self.name_label.pack(side=LEFT)

        self.price_label = Label(self.sub_modify_section_frame_right, text = ":Price")
        self.price_label.pack(side=RIGHT)

        self.name_entrybox = tk.Entry(self.sub_modify_section_frame_right)
        self.name_entrybox .pack(side=LEFT)

        self.price_entrybox = tk.Entry(self.sub_modify_section_frame_right)
        self.price_entrybox .pack(side=RIGHT)

        self.delete_button = tk.Button(self.sub_modify_section_frame_left, text = "Delete",command = lambda :self.delete_item(), bg="seashell3")
        self.delete_button.pack(side=BOTTOM)

        self.populate_modify_section(list(self.menu.keys())[0])

    def create_item(self):
        new_name = self.name_entrybox.get()
        new_price = self.price_entrybox.get()
        try:
            float(new_price)
            price_check = True
        except ValueError:
        ...
        if new_name !="" and price_check:
            section = self.modify_menu_current_section
            try:
                new_item = {'name': new_name, 'price': float(new_price)}
                self.menu[section].append(new_item)
                self.populate_modify_section(section)
                self.populate_section(section)
            except TypeError:
                messagebox.showerror("Creation failed", "Select an item within the section first.")
        else:
            messagebox.showerror("Creation failed", "Either name or price are in the wrong format.")

    def delete_item(self):
      self.menu[self.modify_menu_current_section].pop(self.active_modify_item)
      self.populate_modify_section(self.modify_menu_current_section)
      messagebox.showinfo("Notice", "To see change refresh the section on the main window.")


    def set_current_mod_item(self,input):
        self.active_modify_item = input[0]
        self.modify_menu_current_section = str(input[1])
        self.current_select_item_label.config(text = f"{input[2]} - {input[3]}")

    def populate_modify_section(self, current_section):
        if self.modify_detail_frame.winfo_exists():
            for detail_listed in self.modify_detail_frame.winfo_children():
                detail_listed.destroy()
        ...
        self.modify_menu_current_section = self.menu[current_section]
        
        for main_key, sub_key in enumerate(self.modify_menu_current_section):
        # Creates multi-line string -- Must not be indented
            button_text = f"""{str(sub_key['name'])} 
{str(sub_key['price'])}"""
            
            #formats values into a list
            button_output = (main_key, current_section, sub_key['name'], sub_key['price'])

            button = tk.Button(self.modify_detail_frame,text = button_text,  bg="seashell3",height=2,width=15,command = lambda output = button_output: self.set_current_mod_item(output))
            button.grid(row=position_on_x, column=position_on_y,padx=6,pady=6)
...
root = tk.Tk()
root.title("Sporky Order Managment Application")
